...
# Validate the variables
	...
	if ![regexp -- {^([[:alpha:]]+)\d+.*$} $::x__SHOT void m__SEQ] {
		puts stderr "SHOT(\"$::x__SHOT\"): NOT valid!"
		return -1
	}
	# Contradict?
	if {$::x__SEQ != $m__SEQ} {
...
if {$x_OPT == ""} {
##	puts "USER = \"$::x__USER\", GROUP = \"$::x__GROUP\""
	if {[string toupper $::x__USER] != [string toupper ADministrator] && [string toupper $::x__USER] != [string toupper RenderMan]} {
...
##	puts "USER = \"$::x__USER\", GROUP = \"$::x__GROUP\""
if {[string toupper $::x__USER] == [string toupper ADministrator]} {

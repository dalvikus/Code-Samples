#!/usr/local/tcltk-8.3.4/bin/wish83
...
#	pack $::vars::f__gu.user.u -side left
	...
	set ::vars::menu__mUSER [tk_optionMenu $::vars::f__gu.user.mu ::vars::m__USER $::g__VOID $::g__USER]
	$::vars::f__gu.user.mu configure -width $m_width
	$::vars::menu__mUSER entryconfigure 0 -command "mUSER_selected"
	$::vars::menu__mUSER entryconfigure 1 -command "mUSER_selected"
	# in METHOD_selected procedure
#	pack $::vars::f__gu.user.mu -side left
	# for POP
	# \ / : * ? " < > |
...
#		pack configure $::vars::f__gu.user.u -after $::vars::f__gu.user.l
		pack configure $::vars::f__gu.user.mu -after $::vars::f__gu.user.l
...
#		pack forget $::vars::f__gu.user.u
		pack forget $::vars::f__gu.user.mu
...
proc Run {} {
	...
	if ![regexp -- {^>>>.*<<<$} $m_USER] {
		...
		append m_directory /$::vars::m__GROUP/pub
	}
	...
	if {$::vars::m__METHOD == "PUSH"} {
		...
		foreach file $file_LIST {
			...
			# collect filename
			lappend bare_LIST [file tail $file]
		...
		# send e-mail
		if {[llength $bare_LIST] == 1} {
			set files [lindex $bare_LIST 0]
		} else {
			set files \{[join $bare_LIST ,]\}
		}
		set m_message \
{<HTML>
<HEAD>
...
<PRE>
	cp -i $m_directory/$files <FONT COLOR="#0066CC">D:/maya/projects</FONT>
		...
		SEND_message \
			...
			${::g__USER}@digiscapefx.com \
			[list all@digiscapefx.com] \
			[join $bare_LIST ", "] \
			[subst $m_message] \
	} else {
		# WHICH one(s)?
...
proc SEND_message {MAIL_SERVER FROM TO SUBJECT body} {
	package require smtp
	package require mime

	set token [mime::initialize -canonical TEXT/HTML -encoding 8bit -string $body]

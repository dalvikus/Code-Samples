...
namespace eval alfred {
	...
	namespace eval gen_RIB {
		proc _init {} {
			...
			#-----------------------------------------------------------------#	
			set mtorOption [open $::alfred::g_HEADER::m__pJOB/mtorOption.opt w]
			puts $mtorOption "workspace SetDir torRIBs ${::alfred::g_HEADER::m__pRIB}"
			puts $mtorOption "workspace SetDir torRIBs ${::alfred::g_HEADER::m__pRIB}"
			puts $mtorOption "workspace SetDir torImgs ${::alfred::g_HEADER::m__pIMAGE}"
			puts $mtorOption "workspace SetDir torShadows ${::alfred::g_HEADER::m__pSHADOW}"
			puts $mtorOption "rg dspyName ${::alfred::g_HEADER::g_RIB}"
			puts $mtorOption "rg dspyServer ${::alfred::g_HEADER::g_display_SERVERNAME}"
			puts $mtorOption "rg dspyServerMode ${::alfred::g_HEADER::g_display_MODE}"
			
			if {$m_AUX_CMD != ""} {
				puts $mtorOption $m_AUX_CMD
			}
			close $mtorOption
		}
		proc _main {} {
...
"
	...
	if {$n_genRIB == 1} {
		...
		set doR2r "\"\""
		if {$m_LL != ""} {
			set doLL \"$::alfred::gen_RIB::m__LL\"
		}
		if {$::alfred::gen_RIB::m__CMDFILEs != ""} {
			set doR2R \"$::alfred::gen_RIB::m__CMDFILEs\"
		}		
...
{			# generating RIB: [format %04d $x__fi]
			Task -title {[format %04d $x__fi]: generating RIBs} -subtasks {
				...
				# mtor_maya4
#				RemoteCmd { $::alfred::gen_RIB::g_CMD -cmd genRib $x__fi } -service {pixarNrm} -atmost 1 -tags {netrender}
				RemoteCmd {y:/usr/local/bin/X_MTOR.BAT $::alfred::g_HEADER::m__soSCENE $::alfred::g_HEADER::m__pJOB/mtorOption.opt $x__fi $::alfred::g_HEADER::m__pRIB$::alfred::g_HEADER::g_RIB.[format %04d $x__fi].rib $doLL $doR2R} -service {pixarNrm} -atmost 1 -tags {netrender}
			} -cleanup {
			} -chaser {
				$m_CHASER
			}

...
button $w.buttons.code -text "See Code" -command "xprintf vprintf print_ALL"
...
#### VARIABLEs
	...
	proc vcmd__f_range {w} {
		...
		set RECT [$w.c coords box_C__$ID]
	...
	proc append_LAYER {w} {
		...
		if {[array exists ::ezcomp::layers::$ID] == 1} {
			if [tk_dialog .dialog "Overwrite?" "Overwrite $ID?" "" 0 "Ok" "Cancel"] {
...
bind $w.display.body.directory.entry <Return> \
	"
		::ezcomp::return__directory
		$w.display.body.directory.entry icursor end
		focus -force $w.display.body.directory.entry
	"
#	</BODY: DIRECTORY>
#	<BODY: FILENAME>
...
bind $w.display.body.filename.entry <Return> \
	" \
		::ezcomp::return__filename; \
		$w.display.body.filename.entry icursor end; \
		focus -force $w.display.body.filename.entry
	"
#	</BODY: FILENAME>
#	<BODY: F_RANGE>
...
set ::ezcomp::g__ID xxx
set ::ezcomp::g__f_START 80
set ::ezcomp::g__f_END 110
set ::ezcomp::g__fo //jangbacksan/TMP/images/fo.####.iff
#::ezcomp::init gi
#vwait ::x
#printf $::x
::ezcomp::createPlane $c


focus $w.display.body.uID.entry
# plotDown --
...
Job -title {$::g_DPS_PATHNAME} -comment {
	#
} -envkey {} -dirmaps {} -service {} -tags {} -atleast 1 -atmost 1 -init {
} -subtasks \{
"
...
{			# making TAB sign: [format %04d $x__fi]
			Task -title {[format %04d $x__fi]: making TAB sign} -subtasks {
				...
				RemoteCmd {
					$::SHAKE $m_image_x -addtext \"$::vars::m_show[string toupper $::vars::m_shot 0 0][file extension [file rootname $::g_DPS_PATHNAME]] $::gi::m_LAYER $x__fi\" Arial 24 24 1 32 32 2 1 -setalpha 0 -fo $m_image_o tiff
				} -service {shake} -atmost {1}
			} -cleanup {
			} -chaser {
			}

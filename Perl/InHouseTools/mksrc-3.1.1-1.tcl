...
#
########################################################################
proc ::alfred::_main_TEXT {file n_genRIB n_renHEROs n_ren__GROUND_SHADOWs} {
# 1. open file
...
# 2. write ALFRED job script
	# 2-1. analyze the frame information
	set x_fi $::alfred::g_HEADER::g_fi
	if ![::utils::__parse_fi $::alfred::g_HEADER::g_fi f_START f_END f_STEP] {
		printf "frame infomation: '$::alfred::g_HEADER::g_fi': NOT valid!"
		return
	}
...
Job -title {alfred job script...} -init {
	Assign BIN_PATH y:/usr/local/bin

	Assign f_START $f_START
	...
	Iterate x__fi -from $f_START -to $f_END -by $f_STEP -subtasks {
	} -template {
		Task {#\[format "%04d" \$x__fi\]} -subtasks {
			# generating RIB...
			Task {generating RIB...} -subtasks {
				#
			} -cmds {
				# Cmd {......}
				# RemoteCmd {......}
			...
			# rendering HERO A... (DPS)
			Task {rendering HERO A...} -subtasks {
				Instance {generating RIB...}
			} -cmds {
				# Cmd {......}
				# RemoteCmd {......}
			} -cleanup {
				RemoteCmd {\$BIN_PATH/ECHO.BAT \[expr \$x__fi - \$f_START + 1\]} -service {PixarNrm}
				$xxx
			}
		} -cmds {
			# remove RIB file
	...
	if {$ren_HERO == "A"} {
		set xxx "RemoteCmd {\$BIN_PATH/ECHO.BAT \[expr \$x__fi - \$f_START + 1\]} -service {PixarNrm}"
	} else {
		set xxx #
	}
	puts -nonewline $FILE_ID [subst $m_ALFRED]
	close $FILE_ID
	return
...
Job -title {preview (mtor_maya4 job)} -comment {
	#
} -init {
	Assign output_directory $::alfred::g_HEADER::m__pBASE
	Assign rib_directory $::alfred::g_HEADER::m__pRIB
...
#				RemoteCmd {$::UNIX_MV $m_RIB_NEW $m_RIB_OLD} -service {pixarNrm} -atmost 1 -tags {netrender}
#				RemoteCmd {$::UNIX_BASH --login -c \"r2r $m_RIB_OLD $::alfred::gen_RIB::m__CMDFILEs > $m_RIB_NEW\"} -service {pixarNrm} -atmost 1 -tags {netrender}
				RemoteCmd {$::WINDOWS_CMD /C \"y:/bin/perl.exe y:/usr/local/bin/r2r-1.07 $m_RIB_OLD [join $m_CMDLIST] > $m_RIB_NEW\"} -service {pixarNrm} -atmost 1 -tags {netrender}"
		} else {
			set m_CLEANUP "# ......"
		}
...
{				Task -title {[format %04d $x__fi]: rendering HERO - $m_SET} -subtasks {
					...
					RemoteCmd {
						Y:/usr/local/bin/X_RENDER.BAT \${x}${::alfred::g_HEADER::g_RIB}.[format %04d $x__fi].rib "$m_EDIT_ARGS"
					} -service {pixarNrm} -atmost 1 -tags {netrender}
				} -cleanup {
...
{			# making TAB sign: [format %04d $x__fi]
			Task -title {[format %04d $x__fi]: making TAB sign} -subtasks {
				...
				RemoteCmd {
					$::SHAKE $m_image_x -addtext \"$::alfred::g_HEADER::g_SHOW[string toupper $::alfred::g_HEADER::g_SHOT 0 0][file extension [file rootname $::alfred::mk_DPS::g__DPS_PATHNAME]] $::alfred::g_HEADER::g_LAYER $x__fi\" Arial 24 24 1 32 32 2 1 -setalpha 0 -fo $m_image_o
...
{			# copying to DPS: [format %04d $n__fi]
			Task -title {[format %04d $n__fi]: copying to DPS} -subtasks {
				Task -title {[format %04d $x__fi]: copying to DPS - $n__fi} -subtasks {
					...
					Cmd {
						$::UNIX_BASH --login -c "cp $m_image_o $m_image_in_DPS"
					} -tags {DPSCopy}
				} -cleanup {
				} -chaser {
				}

...
WARN
			...
			if ( $xberm eq 'b' ) {
				push( @g__XBERM_LIST, [ '-', $object_ID, 'b', $display_ID ] );
				push( @g__XBERM_LIST, [ $display_ID, $object_ID, 'e', '' ] );
			} elsif ( $xberm eq 'x' ) {
				push( @g__XBERM_LIST, [ '-', $object_ID, 'x', $display_ID ] );
				push( @g__XBERM_LIST, [ $display_ID, $object_ID, 'r', '' ] );
			} else {
				push( @g__XBERM_LIST, [ $display_ID, $object_ID, $xberm, '' ] );
			}
		}
	}
}
...
#	print STDERR $display_ID . '::' . $attribute_ID . $/;
	my	@indices;
	...
	my	$ix = $indices[ 0 ];
#	print STDERR join( ', ', @indices ) . $/;
#	print STDERR '$ix = ' . $ix . $/;
	if ( $g__XBERM_LIST[ $ix ][ 2 ] eq 'm' ) {	# Matte
#
#	AttributeBegin
...
#
	...
	} elsif ( $g__XBERM_LIST[ $ix ][ 2 ] eq 'b' ) {	# blind
		my	@LIGHT_IDs = ();
		match_LIGHTs( \@indices, \@LIGHT_IDs );
		if ( $#LIGHT_IDs == -1 ) {
			# do nothing with $ref__ATTRIBUTE_CONTENT
		} else {
				${ $ref__ATTRIBUTE_CONTENT } =~ s/(\n(\s*)Attribute "identifier" .*?\n)/ref_LIGHT( $1, $2, \@LIGHT_IDs )/e;
		}
#		unless ( $display_ID ne '-' ) {
#			foreach $ix ( @indices ) {
#				${ $ref__ATTRIBUTE_CONTENT } =~ s/(\n(\s*)Attribute "identifier" .*?\n)/ref_LIGHT( $1, $2, $g__XBERM_LIST[ $ix ][ 3 ] )/e;
#			}
#		} else {
#			# do nothing with $ref__ATTRIBUTE_CONTENT
#		}
	} elsif ( $g__XBERM_LIST[ $ix ][ 2 ] eq 'x' ) {	# blind off
		my	@LIGHT_IDs = ();
		match_LIGHTs( \@indices, \@LIGHT_IDs );
		if ( $#LIGHT_IDs == -1 ) {
			# do nothing with $ref__ATTRIBUTE_CONTENT
	...
	} elsif ( $g__XBERM_LIST[ $ix ][ 2 ] eq 'e' ) {	# expunge
		${ $ref__ATTRIBUTE_CONTENT } = '';
	} elsif ( $g__XBERM_LIST[ $ix ][ 2 ] eq 'r' ) {	# recover
		# do nothing with $ref__ATTRIBUTE_CONTENT
	}
...
sub isMatched
{
	...
	my	$i;
	my	$gi;
	for ( $gi = $#g__XBERM_LIST; $gi >= 0; --$gi ) {
		for ( $i = $#LIST; $i >= 0; --$i ) {
			# 06/25/2002
			$LIST[ $i ] =~ s/_.*//g;

			$display_ID =~ /^$g__XBERM_LIST[ $gi ][ 0 ]$/
				&&
			$LIST[ $i ] =~ /^$g__XBERM_LIST[ $gi ][ 1 ]$/
				&&
			push( @{ $ref_indices }, $gi )
				&&
			last;
...
sub	isMatch_LIGHT
{
	...
	for ( my $i = 0; $i <= $#g__LIGHT_LIST; ++$i ) {
		my	@LIST = split( '\|', $g__LIGHT_LIST[ $i ][ 0 ] );
		shift @LIST unless ( $LIST[ 0 ] ne '' );
		for ( my $ii = $#LIST; $ii >= 0; --$ii ) {
			return $$LIST[ $ii ] =~ /^$LIGHT$/
					&&
...
sub	match_LIGHTs
{
	my	( $ref_indices, $ref__LIGHT_IDs ) = @_;
	my	%included__LIGHT_ID = ();
	my	%excluded__LIGHT_ID = ();

	my	$i;
	my	@LIST;
	my	$ID;
	for ( my $gi = 0; $gi <= $#{ $ref_indices }; ++$gi ) {
		my	$ix = ${ $ref_indices }[ $gi ];
		my	$x = $g__XBERM_LIST[ $ix ][ 2 ];
		next unless ( $x =~ /^[bx]$/ );
		my	$LIGHT = $g__XBERM_LIST[ $ix ][ 3 ];
		$LIGHT =~ s/_.*$//;
#		print STDERR 'index = ' . $ix . ', LIGHT = ' . $LIGHT . $/;

		unless ( $x ne 'b' ) {
			# look for ID for given LIGHT
			for ( $i = 0; $i <= $#g__LIGHT_LIST; ++$i ) {
				@LIST = split( '\|', $g__LIGHT_LIST[ $i ][ 0 ] );
				shift @LIST unless ( $LIST[ 0 ] ne '' );
				for ( my $ii = $#LIST; $ii >= 0; --$ii ) {
					next unless ( $LIST[ $ii ] =~ /^$LIGHT$/ );
					$ID = $g__LIGHT_LIST[ $i ][ 1 ];
					if ( ! exists( $excluded__LIGHT_ID{ $ID } )
						&&
					   ! exists( $included__LIGHT_ID{ $ID } ) ) {
						$excluded__LIGHT_ID{ $ID } = 1;
					}
					last;
				}
			}
		...
		for ( $i = 0; $i <= $#g__LIGHT_LIST; ++$i ) {
			...
			for ( my $ii = $#LIST; $ii >= 0; --$ii ) {
				next unless ( $LIST[ $ii ] =~ /^$LIGHT$/ );
				$ID = $g__LIGHT_LIST[ $i ][ 1 ];
				if ( ! exists( $included__LIGHT_ID{ $ID } )
					&&
				   ! exists( $excluded__LIGHT_ID{ $ID } ) ) {
					$included__LIGHT_ID{ $ID } = 1;
				}
				last;
			}
		}
...
#	print STDERR 'included LIGHT IDs = ' . join( ', ', keys( %included__LIGHT_ID ) ) . $/;
#	print STDERR 'excluded LIGHT IDs = ' . join( ', ', keys( %excluded__LIGHT_ID ) ) . $/;
	@{ $ref__LIGHT_IDs } = keys( %excluded__LIGHT_ID );
}

sub	ref_LIGHT
{
	my	( $ATTR, $TAB, $ref__LIGHT_IDs ) = @_;
	my	$x;
	foreach ( @{ $ref__LIGHT_IDs } ) {
		$x .= $TAB . 'Illuminate ' . $_ . ' 0' . $/;
	}
	return $ATTR
		. '#' x 72 . $/
		. $x
